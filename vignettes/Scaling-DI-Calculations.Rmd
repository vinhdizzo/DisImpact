...
Instead of passing in a student level data set, the user could also pass in a summarized data set (reduce data size).  To do so, the user should also specify `weight_var` to indicate the group size.  Let's illustrate with an example:
...
For the proportionality index (PI) method, DI is determined using `di_prop_index_cutoff=0.8` by default.  This could be changed:
